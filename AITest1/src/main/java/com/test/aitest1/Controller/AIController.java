package com.test.aitest1.Controller;
...
class Client {
    ...
    public ChatCompletionResponse ChatCompletion(ChatCompletionRequest request) throws IOException {
        ...
        OkHttpClient client = new OkHttpClient.Builder()
                .connectTimeout(30000, TimeUnit.SECONDS)//连接超时(单位:秒)
                .callTimeout(120000, TimeUnit.SECONDS)//整个流程耗费的超时时间(单位:秒)--很少人使用
                .pingInterval(50000, TimeUnit.SECONDS)//websocket轮训间隔(单位:秒)
                .readTimeout(60000, TimeUnit.SECONDS)//读取超时(单位:秒)
                .writeTimeout(60000, TimeUnit.SECONDS)//写入超时(单位:秒)
                .build();
...
public class AIController {
    ...
    public ResponseEntity<String> test(@RequestBody String Str) {
        ...
        String apiKey = "sk-8b8ZKz1nCEZ5crMDqLRro2mKIkzTAIrWV7KhJzpud0jtXVZ6";
        ...
        final List<ChatCompletionMessage> messages = List.of(
                ...
                new ChatCompletionMessage(ChatMessageRole.USER.value(),
                        Str+"，请用纯文本格式返回,不要markdown格式，多生成点，生成大概500字，换行记得控制好，输出格式看着好看一点")
        );
        ...
        String result = answer.toString(); // 假设这是您要返回的结果
        return ResponseEntity.ok(result); // 返回结果
    }
    /*@GetMapping("/emprequest")
    public String testAns(@RequestBody String Str) {
        answer.toString();
